...
static void test_vec_znx_big_normalize(VEC_ZNX_BIG_NORMALIZE_BASE2K_F normalize,
  ...
  for (uint64_t sa : {3, 5, 7}) {
    for (uint64_t sr : {3, 5, 7}) {
      ...
      std::vector<uint8_t> tmp_space(normalize_tmp_bytes(module, n));
      normalize(module, n, k, r.data(), sr, r_sl, a.data, sa, tmp_space.data());
    }
  }
...
static void test_vec_znx_big_range_normalize(  //
  ...
  for (uint64_t sa : {6, 15, 21}) {
    for (uint64_t sr : {3, 5, 7}) {
      ...
      std::vector<uint8_t> tmp_space(normalize_tmp_bytes(module, n));
      normalize(module, n, k, r.data(), sr, r_sl, a.data, a_start, a_end, a_step, tmp_space.data());
      fft64_vec_znx_big_normalize_base2k(module, n, k, r2.data(), sr, r_sl, aextr.data, range_size, tmp_space.data());
      for (uint64_t i = 0; i < sr; ++i) {
